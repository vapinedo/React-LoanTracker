...
const useEmpleadoStore = create<EmpleadosState>((set) => ({
  ...
  getEmpleadoOptions: async () => {
    try {
      ...
      set({ loading: false, error: 'Error al obtener empleado options' });
      console.error(error);
    }
  },
  ...
  getEmpleadoById: async (id: string) => {
    try {
      ...
      if (empleado) {
        // Actualizar el estado solo si se encontró el empleado
  ...
  createEmpleado: async (empleado: Empleado) => {
    try {
      ...
      throw error; // Relanzar el error para que lo maneje el componente llamador si es necesario
    }
  },
